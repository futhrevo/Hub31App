...
class ClassHLSSource extends React.Component {
  ...
  onHlsjsError(event, eventData) {
    ...
    if (eventData.fatal) {
      console.error(
        `HLS Fatal Error: ${eventData.type}:${eventData.details}:${eventData.reason
        }`,
      );
    } else {
      console.warn(
