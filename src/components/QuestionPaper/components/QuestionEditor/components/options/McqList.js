  ...
  return (
    <div>
      <div>
        {items.map((opt, index) => (
          <FormGroup key={index}>
            <InputGroup bsPrefix="input-group input-group-margin">
              <InputGroup.Prepend>
                <InputGroup.Radio
                  aria-label="..."
                  name={`question[${id}].mcqAnswer${id}`}
                  onChange={() =>
                    props.setFieldValue(`answers[${id}]`, [index])
                  }
                  checked={answers.indexOf(index) > -1}
                />
              </InputGroup.Prepend>
